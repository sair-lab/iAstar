...
with torch.no_grad():
    ...
    ia_star = iastar(encoder_input=3,
                    ...
                    w=2.0).to(device)
    ia_star.encoder.load_state_dict(torch.load("/home/cxy/SAIRLAB/iAstar/model/09-07-2024-01-36-26/12/iaster1UNet.pkl")["model_state_dict"])
...
with torch.no_grad():
    for env in env_list:
        files = glob.glob(f"./planning-datasets/{env}/instances/064/*.npz")
        
        for filename in files:
            print(filename)
            dataloader = create_dataloader(filename, "train", map_num, shuffle=True)
            ...
            for i in range(n):
                ...
                print("Va finished")
                t_va[i] = t2 - t1
                num_va[i] = va_outputs.histories.sum()
                torch.cuda.empty_cache()
                ia_star.eval()
                t1 = time.time()
                ...
                if map_shape == 64:
                    fw100_traj = outputs_fw100.paths.to(device)
    ...
    save_results(result_dict, "mm0640214.xls")
